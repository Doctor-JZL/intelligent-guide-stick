...
//	Free_Fall_Interrupt();	//自由落体
	Motion_Interrupt();		//运动
//	Zero_Motion_Interrupt();	//静止
...
{
    MPU_Write_Byte(MPU6050_RA_FF_THR,0x30);             //自由落体阈值 
    MPU_Write_Byte(MPU6050_RA_FF_DUR,0x01);             //自由落体检测时间20ms 单位1ms 寄存器0X20
}
void Motion_Interrupt(void)             //运动中断
{
    MPU_Write_Byte(MPU_MOTION_DET_REG,0x43);            //运动阈值 
//	 MPU_Write_Byte(MPU_MOTION_DET_REG, 0X03);//运动检测阀值， 相当于灵敏度吧
    MPU_Write_Byte(MPU6050_RA_MOT_DUR,0x04);            //检测时间20ms 单位1ms 寄存器0X20
}
void Zero_Motion_Interrupt(void)        //静止中断
{
    MPU_Write_Byte(MPU6050_RA_ZRMOT_THR,0x50);          //静止阈值 
    MPU_Write_Byte(MPU6050_RA_ZRMOT_DUR,0x12);          //静止检测时间20ms 单位1ms 寄存器0X20
}
